...
import TeacherGroupsCreate from './componets/TeacherGroupsCreate';
import TeacherEditGr from './componets/TeacherEditGr';
import TeacherDeleteGr from './componets/TeacherDeleteGr';
import { NavLink } from 'react-router-dom';

export default function TeacherOwnGroups() {
  ...
  const teacherId = 1; // Hozirgi kirgan o'qituvchining ID si, backenddan keladi
  ...
  const groups = allGroups.filter(group => group.teacherId === teacherId);
  ...
  return (
    <div className='p-1 md:p-6 mt-[10px] min-h-screen'>
      <div className="flex justify-between items-center flex-wrap gap-2">
        <h1 className="text-3xl font-bold text-gray-800">Mening Guruhlarim</h1>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mt-[20px]">
        <div className="px-6 py-4 border-b border-gray-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Mening guruhlarim</h2>
          </div>
                ...
                <tr key={group.id} className="hover:bg-gray-50 transition-colors duration-150">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex justify-end space-x-2">
                        <NavLink to={`/teacher/groups/person/${group.id}`}>
                          <button 
      ...
      <TeacherGroupsCreate isOpen={openCreateModal} onClose={() => setOpenCreateModal(false)} />
      <TeacherEditGr isOpen={openEditModal} onClose={() => setOpenEditModal(false)} group={selectedGroup} />
      <TeacherDeleteGr isOpen={openDelModal} onClose={() => setOpenDelModal(false)} onDelete={handleDelete} group={selectedGroup} />
